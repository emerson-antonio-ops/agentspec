...
1. **install** — clones/copies pack to `.claude/packs/{name}/`, updates `.claude/packs/.lock.yaml`, applies agents to `.claude/agents/custom/` and KB to `.claude/kb/` (skips existing files — local-wins)
...
- `.claude/sdd/architecture/PACK_SCHEMA.yaml`
